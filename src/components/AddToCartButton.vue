<template>
  <div
  class="add_to_cart"
  @click="getProductCountInCart(product)">
    <!-- <button v-if="isAdded" class="added" @click="buttonFeedback()">Added to cart</button>
    <button v-else @click="buttonFeedback()" class="orange-bck">Add to cart</button> -->

    <button>
      <img class="store_logo" :src="logo" alt="Store logo">
      <p>Add to cart</p>
      <div>
        <!-- <span class="btn_item_count">{{ this.count_in_cart }}</span> -->
        <span class="btn_item_count" :id="'btn_' + btn_index">0</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      buttonFeedback(){
          // console.log("button index", this.btn_index)
      },
      getProductCountInCart(product) {
        this.$emit("takeToCart", product, this.btn_index)
        ...
        //     console.log(item_variants_in_cart[i].count)
        // }
      }
    },
    watch: {
      btn_state: function(){
        // console.log("from watch button index is ", this.btn_index)
        // console.log("from watch Button state is ", this.btn_state)
        this.isAdded = this.btn_state
      //   console.log("from watch is Added is now ", this.isAdded)
      },
      // cart: {
          // handler() {
          //   // this.$store.commit(mutationTypes.SAVE_CART, cart)
          //   // this.$store.commit("saveCartInSession")
          //   // console.log("cart changed too")
